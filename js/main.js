...
const app = Vue.createApp({
    ...
    methods: {
        async doSearch(){  
            ...
            const founInFavorites = this.favorites.get(this.search)

            const shoulRequest = (() =>{
                if(!!founInFavorites){
                    const {lastRequestTime} = founInFavorites
                    const now = Date.now()
            ...
            if(!!founInFavorites && !shoulRequest){
                console.log("Found and we use the cached version");
                return this.result = founInFavorites
            }
            ...
            try {
                ...
                this.result = data
                founInFavorites.lastRequestTime = Date.now()
            } catch (error) {
                this.error = error
            }finally{
                this.search = null
